...
// VERSIÓN 50 - WhatsApp ahora es conexión manual como las otras redes sociales
export default function SocialConnections() {
  ...
  console.log('🚀 VERSIÓN 52 CARGADA - WhatsApp usa Meta v24.0 con response_type=token');

  useEffect(() => {
    loadConnections();

    // Escuchar mensajes del callback de OAuth
    const handleMessage = (event) => {
      ...
      if (data && data.success === false) {
        ...
        alert(`❌ Error al conectar ${data.platform}: ${data.error}`);
        return;
      }
    };
  ...
  const loadConnections = async () => {
    ...
    try {
      ...
      // Obtener información adicional de WhatsApp
  ...
  const connectPlatform = async (platform) => {
    try {
      ...
      switch (platform) {
        case 'instagram':
          // VERSIÓN 84: Instagram Business usa Facebook Graph API
          const igScopes = 'instagram_basic,instagram_manage_messages,instagram_manage_comments,pages_show_list,pages_read_engagement,business_management';
          authUrl = `https://www.facebook.com/v24.0/dialog/oauth?client_id=${credentials.client_id}&redirect_uri=${encodeURIComponent(credentials.redirect_uri)}&scope=${igScopes}&response_type=code&state=${state}`;
          console.log('✅ VERSIÓN 84 - Instagram Business OAuth URL (Facebook Graph API):', authUrl);
          console.log('✅ VERSIÓN 84 - Instagram Scopes:', igScopes);
          break;
        case 'facebook':
          // VERSIÓN 48: Permisos sin email - SOLO permisos aprobados
          authUrl = `https://www.facebook.com/v19.0/dialog/oauth?client_id=${credentials.app_id}&redirect_uri=${encodeURIComponent(credentials.redirect_uri)}&scope=public_profile,pages_read_engagement,business_management&response_type=code&state=${state}`;
          console.log('🔍 VERSIÓN 48 - Facebook OAuth URL:', authUrl);
          console.log('🔍 VERSIÓN 48 - Facebook Scopes:', 'public_profile,pages_read_engagement,business_management');
          break;
        case 'youtube':
          const scopes = encodeURIComponent('https://www.googleapis.com/auth/youtube.readonly https://www.googleapis.com/auth/youtube.upload https://www.googleapis.com/auth/youtube.force-ssl');
          authUrl = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${credentials.client_id}&redirect_uri=${encodeURIComponent(credentials.redirect_uri)}&scope=${scopes}&response_type=code&access_type=offline&prompt=select_account&state=${state}`;
          break;
        case 'tiktok':
          authUrl = `https://www.tiktok.com/auth/authorize/?client_key=${credentials.client_key}&redirect_uri=${encodeURIComponent(credentials.redirect_uri)}&scope=user.info.basic,video.publish,video.list&response_type=code&state=${state}`;
          break;
        case 'whatsapp':
          // ✅ VERSIÓN 52: WhatsApp usa v24.0 y response_type=token (según ejemplo de Meta)
          const whatsappScopes = 'whatsapp_business_management,whatsapp_business_messaging';
          authUrl = `https://www.facebook.com/v24.0/dialog/oauth?client_id=${credentials.client_id}&redirect_uri=${encodeURIComponent(credentials.redirect_uri)}&scope=${whatsappScopes}&response_type=token&state=${state}`;
          console.log('✅ VERSIÓN 52 - WhatsApp OAuth URL (Meta v24.0):', authUrl);
          console.log('✅ VERSIÓN 52 - WhatsApp Scopes:', whatsappScopes);
          break;
        case 'shopify':
  ...
  const platforms = [
    ...
    },
    {
      ...
      account: 'csn703-10.myshopify.com',
    },
  ];
              ...
              <div className="p-6">
                {isConnected ? (
                  <div className="space-y-3">
                    ...
                    {/* Información adicional para WhatsApp */}
